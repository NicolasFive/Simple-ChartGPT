<template>
  <v-layout>
    <v-navigation-drawer color="grey-darken-2" permanent>
      <v-container>
        <v-row>
          <v-btn variant="flat" prepend-icon="mdi-plus" block rounded color="white" @click="addChat">
            New Chat
          </v-btn>
        </v-row>
        <v-row v-for="it in chats" :key="it.id">
          <v-card class="card-body" prepend-icon="mdi-file" hover rounded>
            <template v-slot:title>
              <div class="card-text">
                {{ it.messages.length>0?it.messages[0].content:"A New Chat" }}
                <v-btn-toggle divided class="card-btn">
                  <v-btn density="compact" icon="mdi-pencil" size="small" :active="false"
                         @click="selectChat(it)"></v-btn>
                  <v-btn density="compact" icon="mdi-delete" size="small" @click="delChat(it)"></v-btn>
                </v-btn-toggle>
              </div>
            </template>
          </v-card>
    ...
    <v-main>
      <v-container class="messages" v-mutate.child="onMutate">
        <template v-if="curChat">
          <Message v-for="it in curChat.messages" :data="it" :key="it.content"></Message>
        </template>
      </v-container>
    </v-main>
        ...
        <v-row no-gutters>
          <v-col align-self="center" cols="10">
            <v-textarea :loading="send.loading" density="compact" variant="solo" type="text"
                          ...
                          auto-grow
                          max-rows="5"
                          hint="Send message what you want to ask ChatGPT,for example 'Show me a maze code of Java.'"
  ...
  if (!send.loading) {
    ...
    try {
      ...
      const {data, error} = await sendMessage(Object.assign(useSettings().value,{
        "messages": JSON.parse(JSON.stringify(curChat.value.messages))
      }))
      if (error.value) {
...
onMounted(() => {
  chats = getChats()
  cur = getCur()
  curChat = computed(() => chats.value[cur.value])
  user = useUser()
  watch([chats, cur], () => {
    saveChats()
  }, {deep: true})
